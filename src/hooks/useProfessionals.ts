...
import { getFirestoreInstance, resetFirestoreConnection, isFirestoreInternalError, ensureFirestoreReady } from '../utils/firebase'; // Use getter function instead of direct import
...
const professionalsCache = new Map<string, { data: Professional[], timestamp: number }>();
...
export const useProfessionals = (filterType?: 'mental' | 'sexual') => {
  ...
  useEffect(() => {
    ...
    console.log(`🔍 Setting up professionals subscription with filter: ${filterType || 'all'}`);
    
    // CRITICAL: Clear caches first to ensure fresh data after deletion
    clearAllCaches();
    
    // Generate unique listener ID to prevent conflicts
    const listenerId = `professionals_${filterType || 'all'}_${++listenerIdCounter}_${Date.now()}`;
    listenerIdRef.current = listenerId;
    
    // Clean up any existing listener for this cache key
    const existingListenerId = Array.from(activeListeners.keys()).find(id => id.startsWith(`professionals_${filterType || 'all'}_`));
    ...
    limit(50)
        ...
        results = results.map(professional => {
          if (professional.availability && Array.isArray(professional.availability)) {
            const needsMigration = professional.availability.some(avail => 
              !avail.slots || !Array.isArray(avail.slots) || avail.slots.length === 0
            );
            
            if (needsMigration) {
              console.log(`🔧 Migrating availability for ${professional.name} in memory...`);
              professional.availability = migrateAvailabilityData(professional.availability);
            }
          }
          return professional;
        });
        ...
        console.log(`✅ Fetched ${sortedResults.length} professionals with fallback (fresh data after deletion)`);
...
setProfessionals(sortedResults);
          ...
          professionalsCache.set(cacheKey, { data: visibleProfessionals, timestamp: Date.now() });
        }
      } catch (err) {
    ...
    ensureFirestoreReady()
      .then((isReady) => {
        if (!isReady) {
          console.warn('⚠️ Firestore not ready for professionals subscription, using fallback');
        ...
        try {
          ...
          ensureFirestoreReady().catch(error => {
            console.warn('⚠️ Failed to ensure Firestore ready before professionals subscription:', error);
          });
          
          const db = getFirestoreInstance();
          if (!db) {
            console.warn('⚠️ Firestore not available for professionals subscription, using fallback');
            fetchProfessionalsWithFallback();
            return;
          }
  ...
  q = query(
    collection(db, 'professionals')
          ...
          console.log(`📡 Setting up real-time subscription with ID: ${listenerId}...`);
          ...
          const unsubscribe = onSnapshot(
            ...
            (querySnapshot) => {
              if (!isMountedRef.current || listenerIdRef.current !== listenerId) {
                console.log('🚫 Ignoring snapshot for unmounted component or old listener');
                return;
              }
              
              try {
                console.log(`📊 Received ${querySnapshot.docs.length} documents from Firestore (listener: ${listenerId})`);
                ...
                results = results.map(professional => {
                  if (professional.availability && Array.isArray(professional.availability)) {
                    ...
                    if (needsMigration) {
                      console.log(`🔧 Migrating availability for ${professional.name} in real-time...`);
                      professional.availability = migrateAvailabilityData(professional.availability);
                    }
                  }
                  return professional;
                ...
                console.log(`✅ Processed ${sortedResults.length} professionals successfully (listener: ${listenerId})`);
                
                if (isMountedRef.current && listenerIdRef.current === listenerId) {
setProfessionals(sortedResults);
                  setLoading(false);
                  // Cache the results (fresh cache)
                professionalsCache.set(cacheKey, { data: sortedResults, timestamp: Date.now() });
                  // Clear error if we successfully receive data
                  if (sortedResults.length >= 0) { // Accept 0 results as valid (empty database)
                    setError(null);
                  }
                }
                ...
                if (isMountedRef.current && listenerIdRef.current === listenerId) {
                  setError('Erreur lors du traitement des données des professionnels');
                  setLoading(false);
            ...
            async (err) => {
              if (!isMountedRef.current || listenerIdRef.current !== listenerId) {
                console.log('🚫 Ignoring error for unmounted component or old listener');
                return;
              }
              
              console.error(`❌ Error in professionals subscription (listener: ${listenerId}):`, err);
              
              // CRITICAL: Handle Firestore internal assertion failures
              if (isFirestoreInternalError(err)) {
                console.error('🚨 Firestore internal assertion failure in professionals subscription, resetting connection...');
                ...
                try {
                  ...
                  console.log('✅ Firestore connection reset after internal assertion failure in professionals');
                  ...
                  console.warn('⚠️ Could not reset Firestore after internal assertion failure:', resetError);
                }
              ...
              let errorMessage = 'Erreur lors du chargement des professionnels. Vérifiez votre connexion internet.';
              
              if (err.code === 'permission-denied') {
                errorMessage = 'Accès refusé aux données des professionnels. Veuillez vous reconnecter.';
              } else if (err.code === 'unavailable') {
                errorMessage = 'Service temporairement indisponible. Veuillez réessayer dans quelques instants.';
              } else if (err.code === 'failed-precondition') {
                console.warn('⚠️ Firestore index may be required, but continuing with basic query');
                errorMessage = 'Chargement en cours... Les données peuvent prendre quelques instants à apparaître.';
              } else if (err.message && err.message.includes('Target ID already exists')) {
                console.error('🎯 Target ID conflict detected, cleaning up...');
                errorMessage = 'Conflit de cache détecté. Rechargement automatique en cours...';
          ...
          const timeoutId = setTimeout(() => {
            if (isMountedRef.current && loading && listenerIdRef.current === listenerId) {
              console.warn(`⚠️ Professionals subscription timeout for listener: ${listenerId}`);
              setLoading(false);
              // Don't set error for empty database - this is expected after deletion
              console.log('ℹ️ No professionals found - this is expected after database cleanup');
            }
          ...
          return () => {
            ...
            if (listenerIdRef.current && activeListeners.has(listenerIdRef.current)) {
              console.log(`🧹 Cleaning up professionals listener: ${listenerIdRef.current}`);
            ...
            console.log(`🧹 Cleaned up professionals subscription for: ${listenerId}`);
          };
        } catch (error) {
          console.error('❌ Error setting up professionals subscription:', error);
          if (isMountedRef.current) {
            setError('Erreur lors de la connexion à la base de données. Veuillez réessayer.');
            setLoading(false);
      ...
      .catch((error) => {
        console.error('❌ Failed to ensure Firestore ready for professionals subscription:', error);
        if (isMountedRef.current) {
          setError('Problème de connexion à la base de données. Veuillez vérifier votre connexion internet.');
          setLoading(false);
...
export const cleanupAllProfessionalsListeners = () => {
  console.log(`🧹 Cleaning up all ${activeListeners.size} professionals listeners`);
