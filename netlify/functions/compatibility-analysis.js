...
exports.handler = async (event) => {
  ...
  // ── Reserve Koris ──
  const reservation = await reserveKoris(user.uid, 'compatibility')
  ...
  for (let attempt = 0; attempt < MODELS.length; attempt++) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ narrative: narrative || '', koris_debited: true }) }
      }
  ...
  return {
    statusCode: 500,
    headers,
    body: JSON.stringify({ error: 'Tous les modèles sont indisponibles', error_code: 'ai_unavailable', koris_debited: false })
  }
}
