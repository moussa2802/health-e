...
export const joinInfo = onCall(
  {
    region: REGION,
  },
  async (request): Promise<JoinInfoResponse> => {
    const { token } = request.data as JoinInfoRequest;
    ...
    try {
      ...
      if (bookingData?.status !== "confirmed") {
        console.log(
          `[joinInfo] Booking not confirmed: ${bookingId}, status: ${bookingData?.status}`
        );
        return { status: "invalid" };
      }
      ...
      const startsAt = bookingData.startsAt as admin.firestore.Timestamp;
      const endsAt = bookingData.endsAt as admin.firestore.Timestamp;
      const professionalName =
        bookingData.professionalName || "votre professionnel";
      ...
      const startsAtDt = DateTime.fromMillis(startsAt.toMillis()).setZone(TZ);
      const endsAtDt = DateTime.fromMillis(endsAt.toMillis()).setZone(TZ);
